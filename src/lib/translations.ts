...
export const translations = {
  KR: {
    ...
    localApiKeyHelpUpdated: "이곳에 API 키를 저장해두면 필요시 참조할 수 있습니다. 여기에 저장된 키가 있으면 API 호출 시 우선적으로 사용되며, 비어있을 경우 서버의 기본 키가 사용됩니다. 비우고 저장하면 제거됩니다.",
  ...
  JP: {
    ...
    localApiKeyHelpUpdated: "ここにAPIキーを保存しておくと、必要に応じて参照できます。ここに保存されたキーがある場合、API呼び出し時に優先的に使用され、空の場合はサーバーのデフォルトキーが使用されます。空にして保存すると削除されます。",
    adminNameLabel: "管理者名",
